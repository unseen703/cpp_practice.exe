...
int fun(int i, int pre, int x, vi &v , vector<vi> &dp)
{
    if (i < 0)
        return 0;
...
int mod = 1e9 +7;
    ...
    if (v[i] != 0)
    {
        ...
        if ( pre == x+1 || abs(v[i] - pre) <= 1)
            return dp[i][pre] = fun(i - 1, v[i], x, v,dp)%(mod);
    ...
    if(pre == x+1){
        int cnt = 0;
        for( int j = 1; j<= x; j++ )  cnt = (cnt + fun(i-1, j,x, v , dp))%mod;
        return  dp[i][pre] = cnt;
    }
    int cnt = 0;
    for (int j = -1; j <= 1; j++)
    {
        if ( (pre + j <= x && pre + j >= 1))
            cnt = (cnt + fun(i - 1, pre + j, x, v, dp))%(mod);
    }
...
void DKK()
{
    ...
    vector<vi> dp(n+1, vi(x+2, -1));
    cout << fun(n - 1, x+1, x, v, dp);

    // vector<int> dp(n+1, vi( x+1, 0));
}
