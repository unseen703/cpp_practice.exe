...
ll cntstr(int k, int cnt)
{
    ...
    while (count_m < k - 1)
    {
        // if(k == 3) cout<<ans<<" cnount needed "<<count_m<<" count can be "<<k-1<<" " <<nCr(2,2)<<"\n";
        ans = (ans + nCr(k - 1, count_m)) % mod;
        if(k == 3) cout<<ans<<" cnount needed "<<count_m<<" count can be "<<k-1<<" " << nCr(k - 1, count_m)<<"\n";
        count_m++;
    }
    ans++
    return ans;
...
void DKK()
{
    ...
    rep(i, 0, n)
    {
        if (s[i] == '1')
            ...
            cnt2++;
        if (s[i] == '1' && cnt1 == i + 1)
            ans = (ans + ((int)pow(2, cnt1 - 1) % mod)) % mod;
        else if (s[i] == '0' && cnt2 == i + 1)
            ans = (ans + ((int)pow(2, cnt2 - 1)) % mod) % mod;
        else
        {
            if (s[i] == '1')
                ans = (ans + cntstr(i + 1, cnt1)) % mod;
            else
                ans = (ans + cntstr(i + 1, cnt2)) % mod;
        }
        // cout<<ans<<"\n";
    }
    cout << ans << "\n";
