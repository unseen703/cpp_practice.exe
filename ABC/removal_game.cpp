...
long long dp[5000][5000][2];
    ...
    long long fun(int st, int en, int flg, vi &v  ){
        if( st > en) return 0LL;
        if(st == en ) {
            if(flg ) return -1LL*v[st];
            else return v[st];
        }
        if(dp[st][en][flg] != -1) return dp[st][en][flg];

        long long ft = fun(st +1, en , !flg , v) ;
        long long lt = fun(st, en-1,!flg, v );
        if( flg ){
            ft -= v[st];
            lt -= v[en];
            return dp[st][en][flg]  = min(ft , lt);
        }
    
            ft += v[st];
            lt  += v[en];
            return dp[st][en][flg] =  max(ft, lt);
