...
void DKK()
 { 
    int k , n;
    cin >>n>>k;
    auto cmp =  [](const pii x1,  const pii y1){return (x1.ss < y1.ss?1:(x1.ss== y1.ss)?x1.ff<y1.ff:0);} ;
    ...
    sort(all(s), cmp);
           ...
           multiset<int> q;
                rep( i , 0 ,k) q.insert(0);

                // shows for how long k members will be busy are going to be busy
                //  top element will represent latest free memebr of the 
                // for(auto p:s) cout<<p.ff<<' '<<p.ss<<"\n";
           int cnt =0;
           int ans =0;
       int i =0;
    //    auto it = s.begin();
        ...
        if(i == s.size() ) break;
            ...
            int st = (s[i]).first;
            // cout<<st<<"\n";
            auto it1 = q.upper_bound(st);
            if(it1 == q.begin()) {i++; continue;}
            it1--;
            ans++;
            // (*it1) = (*it).ss;
            // cout<<(*it1)<<" 2\n";
