...
typedef  long long   ll; typedef  pair<int, int>  pii;  typedef  vector<int>  vi;   typedef  vector<ll>  vll;  typedef  vector<pii>  vpi; typedef map<int, int> mii;
...
x[0] = v[0], y[0] = v[0];
   ...
   rep( i , 1, n-1){
    ...
    ll sum1 = min(sumx + y[i-1]*x[i], sumy + x[i-1]*x[i] );
    ll sum2 = min(sumx + y[i-1]*y[i], sumy + x[i-1]*y[i] );
    sumx = sum1 , sumy = sum2;
    // cout<<sumx<<" "<<sumy<<"\n";
   }
